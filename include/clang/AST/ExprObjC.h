...
class ObjCArrayLiteral final : public Expr, private llvm::TrailingObjects<ObjCArrayLiteral, Expr*> {
...
class ObjCDictionaryLiteral final : public Expr, private llvm::TrailingObjects<ObjCDictionaryLiteral, ObjCDictionaryLiteral_KeyValuePair, ObjCDictionaryLiteral_ExpansionData> {
...
public:
  ...
  ObjCDictionaryElement getKeyValueElement(unsigned Index) const {
    ...
    if (HasPackExpansions) {
      const ExpansionData &Expansion = getTrailingObjects<ExpansionData>()[Index];
  ...
  child_range children() {
    ...
    return child_range(reinterpret_cast<Stmt **>(getTrailingObjects<KeyValuePair>()),
                       reinterpret_cast<Stmt **>(getTrailingObjects<KeyValuePair>()) + NumElements * 2);
  }
...
class ObjCMessageExpr final : public Expr, private llvm::TrailingObjects<ObjCMessageExpr, void *, SourceLocation> {
...
public:
  ...
  const Expr * const *getArgs() const {
    return reinterpret_cast<const Expr * const *>(getTrailingObjects<void *>() + 1);
  }
...
/// \endcode
class ObjCBridgedCastExpr final : public ExplicitCastExpr, private llvm::TrailingObjects<ObjCBridgedCastExpr, CXXBaseSpecifier*>  {
