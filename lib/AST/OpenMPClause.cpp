...
void OMPPrivateClause::setPrivateCopies(ArrayRef<Expr *> VL) {
  ...
  std::copy(VL.begin(), VL.end(), getTrailingObjects<Expr *>() + varlist_size());
}
...
void OMPFirstprivateClause::setInits(ArrayRef<Expr *> VL) {
  ...
  std::copy(VL.begin(), VL.end(), getTrailingObjects<Expr *>() + 2 * varlist_size());
}
...
void OMPLastprivateClause::setPrivateCopies(ArrayRef<Expr *> PrivateCopies) {
  assert(PrivateCopies.size() == varlist_size() &&
         "Number of private copies is not the same as the preallocated buffer");
  std::copy(PrivateCopies.begin(), PrivateCopies.end(), getTrailingObjects<Expr *>() + varlist_size());
}
...
void OMPLastprivateClause::setSourceExprs(ArrayRef<Expr *> SrcExprs) {
  assert(SrcExprs.size() == varlist_size() && "Number of source expressions is "
                                              "not the same as the "
                                              "preallocated buffer");
  std::copy(SrcExprs.begin(), SrcExprs.end(), getTrailingObjects<Expr *>() + 2 * varlist_size());
}
...
void OMPLastprivateClause::setDestinationExprs(ArrayRef<Expr *> DstExprs) {
  ...
  std::copy(DstExprs.begin(), DstExprs.end(), getTrailingObjects<Expr *>() + 3 * varlist_size());
}
...
void OMPLastprivateClause::setAssignmentOps(ArrayRef<Expr *> AssignmentOps) {
  ...
  std::copy(AssignmentOps.begin(), AssignmentOps.end(),
            getTrailingObjects<Expr *>() + 4 * varlist_size());
}
