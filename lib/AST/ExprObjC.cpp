...
ObjCDictionaryLiteral::Create(const ASTContext &C,
                              ...
                              bool HasPackExpansions, QualType T,
                              ObjCMethodDecl *method, SourceRange SR) {
  void *Mem = C.Allocate(totalSizeToAlloc<KeyValuePair, ExpansionData>(VK.size(), HasPackExpansions ? VK.size() : 0));
...
ObjCDictionaryLiteral::CreateEmpty(const ASTContext &C, unsigned NumElements,
  ...
  void *Mem = C.Allocate(totalSizeToAlloc<KeyValuePair, ExpansionData>(NumElements, HasPackExpansions ? NumElements : 0));
  return new (Mem)
...
ObjCMessageExpr *ObjCMessageExpr::alloc(const ASTContext &C, unsigned NumArgs,
  ...
  return (ObjCMessageExpr *)C.Allocate(
      totalSizeToAlloc<void *, SourceLocation>(NumArgs + 1, NumStoredSelLocs), llvm::AlignOf<ObjCMessageExpr>::Alignment);
}
