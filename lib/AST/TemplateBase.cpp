...
void ASTTemplateArgumentListInfo::initializeFrom(const TemplateArgumentListInfo &Info,
                                                 bool &Dependent, 
...
ASTTemplateKWAndArgsInfo::Create(ASTContext &C,
                                 SourceLocation TemplateKWLoc,
                                    const TemplateArgumentListInfo &List) {
  std::size_t size = ASTTemplateKWAndArgsInfo::sizeFor(List.size());
  void *Mem = C.Allocate(size, llvm::alignOf<ASTTemplateKWAndArgsInfo>());
  ASTTemplateKWAndArgsInfo *TAI = new (Mem) ASTTemplateKWAndArgsInfo();
  TAI->initializeFrom(TemplateKWLoc, List);
  return TAI;
}
...
void ASTTemplateKWAndArgsInfo::copyInto(
                                      TemplateArgumentListInfo &Info) const {
  Info.setLAngleLoc(LAngleLoc);
  Info.setRAngleLoc(RAngleLoc);
  for (unsigned I = 0; I != NumTemplateArgs; ++I)
